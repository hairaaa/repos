...
def brithAttack():#随机生成两个长度为128bit的数据进行生日攻击使得两串数据在进行了sm3之后杂凑值相同
    #在此只进行简单的操作，比对杂凑值的前16bit
    # 初始化两个列表
    list_x = []
    # 生成 sqrt(p) 长度的随机数集合
    list_x = getRandomList(1024)
    for i in range(512):
        # 计算出它们的杂凑值
        message1 = list_x[i]
        message2 = list_x[i+512]
        ...
        if(result1[:4]==result2[:4]):
            ...
            print("1:", message1)
            print("2:", message2)
            print("杂凑值前16位为", result1[:4])
            print("杂凑值前16位为", result2[:4])
            return 1
